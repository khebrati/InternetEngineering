...
exports.note_detail = asyncHandler(async (req, res, next) => {
    ...
    const note = await Note.findOne({ "_id": req.params.id }, "title date text ")
        .exec();
    //console.log(note);
    res.render('note_view', { "note":note })
});

exports.note_create_post = asyncHandler(async (req, res, next) => {
    await new Note({ "title": req.body.title, "text": req.body.text }).save();
